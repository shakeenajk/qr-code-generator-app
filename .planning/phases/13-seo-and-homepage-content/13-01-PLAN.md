...
<task type="auto">
  ...
  <action>
    ...
    2. In src/layouts/Layout.astro make exactly TWO changes:

       Change A — Fix JSON-LD type on line 23:
       Change `'@type': 'WebApplication'` to `'@type': 'SoftwareApplication'`
       Also update the comment on line 77 from "WebApplication schema" to "SoftwareApplication schema".
       All other fields in `webAppSchema` remain identical (name, url, description, applicationCategory, operatingSystem, browserRequirements, offers).
  ...
  <done>Layout.astro emits SoftwareApplication JSON-LD and a sitemap link; astro-seo is installed.</done>
</task>
...
3. `node -e "import('./src/data/useCases.js')"` or TypeScript check confirms 6 entries
